...
<html>
	<head>
		...
		<link rel="icon" type="image/png" sizes="192x192" href="https://app.knotel.com/favicon/android-icon-192x192.png">
		<link rel="icon" type="image/png" sizes="32x32" href="https://app.knotel.com/favicon/favicon-32x32.png">
		<link rel="icon" type="image/png" sizes="96x96" href="https://app.knotel.com/favicon/favicon-96x96.png">
		<link rel="icon" type="image/png" sizes="16x16" href="https://app.knotel.com/favicon/favicon-16x16.png">
		...
		<style type="text/css">/**
			...
			*/max-width: 700px; padding: 0px 0px;
			  ...
			  return regex.test(email);
                ...
                $(document).ready(function(){
                    $(document).find('#addGuest').click(function(){
                        ...
                        var num = ($('ul#addGuestList li').length + 2);
                        $('#EmailError').hide();
                        $('span#guestCount').text(num);
                        $('ul#addGuestList').append('<li class="guestUserLI" name="'+name+'" email="'+email+'" data-radium="true" style="overflow: hidden; cursor: pointer; width: 100%; display: inline-flex;"><div title="'+(num - 1)+'. '+name+' ('+email+') " data-radium="true" style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden; float: left; width: calc(100% - 40px); line-height: 24px;">'+(num - 1)+'. '+name+' <'+email+'> </div><div class="guestUserLISvg"><svg viewBox="0 0 24 24" style="-moz-user-select: none; display: inline-block; color: rgba(0, 0, 0, 0.87); fill: currentcolor; height: 24px; width: 24px; transition: all 450ms cubic-bezier(0.23, 1, 0.32, 1) 0ms; cursor: pointer; float: right;"><path d="M14.59 8L12 10.59 9.41 8 8 9.41 10.59 12 8 14.59 9.41 16 12 13.41 14.59 16 16 14.59 13.41 12 16 9.41 14.59 8zM12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg></div></li>');
                        $('input#guestName').val('');
                ...
                $(document).on('click','.guestUserLISvg',function(){
                    $(this).parent().remove();
                    var num = (parseInt($('span#guestCount').text()) - 1);
                    $('span#guestCount').text(num);
                });
                ...
                $(document).on('keyup','#guestName',function() {
                	if($.trim($(this).val()) == '' || $.trim($('input#guestEmail').val()) == '' || !isEmail($('input#guestEmail').val())) {
                		$('button#addGuest').attr('disabled','disabled').css({'background':'rgb(224, 224, 224)', 'cursor': 'not-allowed'});
                	} else{
                		$('button#addGuest').removeAttr('disabled',false).css({'background':'#bb9f7d', 'cursor': 'pointer'});
                	}
				});


				$(document).on('keyup','#guestEmail',function(){
                	if($.trim($(this).val()) == '' || $.trim($('input#guestName').val()) == '' || !isEmail($(this).val())) {
                		$('button#addGuest').attr('disabled','disabled').css({'background':'rgb(224, 224, 224)', 'cursor': 'not-allowed'});
                	} else{
                		$('button#addGuest').removeAttr('disabled',false).css({'background':'#bb9f7d', 'cursor': 'pointer'});
                	}
                ...
                $(document).on('keyup','#submitterName',function(){
                	if($.trim($(this).val()) == '' || $.trim($('input#submitterEmail').val()) == '' || !isEmail($('input#submitterEmail').val()) ) {
                	...
                	if($.trim($(this).val()) != '' && $.trim($('input#submitterEmail').val()) != '' && isEmail($('input#submitterEmail').val()) ) {
                		...
                		var num = ($('ul#addGuestList li').length);
                		$('span#guestCount').text(num);
                	}

                });
				$(document).on('keyup','input#submitterEmail',function(){
					if($.trim($(this).val()) == '' || $.trim($('input#submitterName').val()) == '' || !isEmail($(this).val()) ) {
                		$('button#guestCountButton').attr('disabled','disabled').css({'background':'rgb(224, 224, 224)', 'cursor': 'not-allowed'});
                	} else{
                		$('button#guestCountButton').removeAttr('disabled',false).css({'background':'#bb9f7d', 'cursor': 'pointer'});
                	}
                ...
                $(document).on('click','button#guestCountButton',function(ent){
                            ...
                            var emailtoGuest = 1;
                            /*if( $(document).find('#emailtoguest').is(':checked') ){
                            	var emailtoGuest = 1;
                            }*/
                            $.ajax({
                                type: 'POST',
                                url: "<?php echo get_home_url(); ?>/wp-admin/admin-ajax.php",
                                data: {'emailtoGuest':emailtoGuest,'evtid':idEvt,'submitterName':submitterName,'submitterEmail':submitterEmail,'guest':guest,
                                    'action': "addguests"
                                },
                            	...
                            	if(avilable == 1){
                               		...
                               		$('div#rsvpForm').after('<h2>You have booked successfully</h2>').remove();
                               	}
                            }
                        });
	...
	<body>
		...
		<div id="render-target" style="width:100%; height:100%;">
			<?php  
				...
				$url = SETAPI_URL."/publications/eventByUrlName/".end($url);
				$ch = curl_init();
				...
				foreach($response as $key => $events){
				    foreach($events as $event){
                                        ...
                                        $utc = new DateTimeZone('UTC');
                                                ...
                                                $monthabv1 = date("M",$eventday);
                                                $day = date("d",$eventday);
                                                $day1 = date("d",$eventday);
                                                $eventyear = date("Y",$eventday);
                                                $eventyear1 = date("Y",$eventday);
                                                $todaydate = date("Y-m-d");
                                                $eventdate = date("Y-m-d",strtotime($event['startDate']));  
                                                $eventdate1 = date("Y-m-d",strtotime($event['startDate']));  

                                                $property_id = $event['propertyId'];
                                                $url_location = SETAPI_URL."/publications/roomsAndPropertiesForEvents/";
                                                $ch = curl_init();
                                                 curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                                                 curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                                                 ...
                                                 $limitRsvp = (isset($event['rsvp']['limit'])) ? $event['rsvp']['limit'] : 0;
                                                 $attendees = count($event['attendees']);
                                                 $topPropertyId = $event['_id'];
//                                                echo "<pre>";
//                                                 print_r($event);
//                                                 die;
                                                 foreach($event['attendees'] as $key =>$val){
                                                    if($val['guestCount'] > 0){
                                                        $attendees = $val['guestCount'] + $attendees;
                                                    }     
                                                 }
			...
			<div data-radium="true" data-reactroot="">
				...
				<input type="hidden" name="eventID" value="<?php echo $event['_id'] ?>" id="eventId">
					<div class="container-fluid">
                        ...
                        <div class="container event-detail-main" style="margin-bottom:40px;">
						<div class="row">
							<div class="col-xs-12 col-md-7">
								<p style="color: rgb(187, 159, 125); font-size: 48px; text-transform: uppercase; line-height:55px; margin-bottom:5px; padding: 0px; margin-top:48px;" class="gtm-black event_main_title"><?php echo $event['title']; ?></p>
								<p style="color: rgb(187, 159, 125); font-size: 14px; text-transform: uppercase; margin:0px 0 20px 0px; padding: 0px;" class="gtm-medium">
								...
								Knotel Private Space, <a  style="text-decoration: underline;" href="<?php echo get_home_url(); ?>/<?php echo preg_replace('/\s+/', '-', strtolower($urlNamer)); ?>" >
										  <?php echo $urlNamer; ?></a>
                                                                ...
                                                                ?>
                                                                 Knotel, <a  style="text-decoration: underline;" href="<?php echo get_home_url(); ?>/<?php echo preg_replace('/\s+/', '-', strtolower($urlNamer)); ?>" >
										  <?php echo $urlNamer; ?></a>       
													...
													<div style="" spellcheck="true" class="public-DraftEditor-content12" contenteditable="false">
														<div data-contents="true">
															<div data-offset-key="921b-0-0" data-editor="32m6v" data-block="true" class="RichTextEditor__block___2Vs_D RichTextEditor__paragraph___3NTf9">
																<div class="public-DraftStyleDefault-block public-DraftStyleDefault-ltr event-img" data-offset-key="921b-0-0"><span data-offset-key="921b-0-0"><span data-text="true">
																<?php if(isset($event['description']) && !empty($event['description'])) { echo $event['description']; } ?></span></span></div>
															</div>
                                ...
                                $two_past_midnight = new DateTime($date, $utc);
                                $two_past_midnight->setTimeZone($amla);
                                $currentTime = $two_past_midnight->format('Y-m-d H:i:s');
                                if( strtotime($event['startDate']) > strtotime($currentTime) ) { 
                                    if($check_property != 'Private Space'){
                                	 ?>
                                
								<div data-radium="true" id="rsvpForm">
									<div style="max-width: 700px; padding: 30px 0 0 0px;" data-radium="true">
										...
										<form>
											...
											<div data-radium="true">
												<p class="gtm-medium" style="margin-bottom: 10px;margin-top: 10px; " data-radium="true">Add guests</p>
												<div style="padding: 0px 8px; background: rgb(245, 245, 245) none repeat scroll 0% 0%; width: 100%; display: flex; align-items: center; height: 68px;" data-radium="true" class="rmq-5380fc94">
													<div style="width: calc(100% - 108px); float: left;" data-radium="true" class="rmq-99aefc51">
														<div style="font-size: 14px; line-height: 22px; width: 48%; height: 40px; display: inline-block; position: relative; background-color: transparent;  transition: height 200ms cubic-bezier(0.23, 1, 0.32, 1) 0ms; cursor: auto;">
														...
														<div style="font-size: 14px; line-height: 22px; width: 48%; height: 40px; display: inline-block; position: relative; background-color: transparent; transition: height 200ms cubic-bezier(0.23, 1, 0.32, 1) 0ms; cursor: auto; float: right;">
															...
															<input style="-moz-appearance: textfield; padding: 0px; position: relative; width: 100%; border: medium none; outline: medium none; background-color: transparent; color: rgba(0, 0, 0, 0.87); cursor: inherit; font: inherit; height: 100%;" id="guestEmail" type="text" placeholder="Email (required)"><!-- react-text: 206 --><!-- /react-text -->
													...
													<button disabled="disabled" id="addGuest" style="-moz-user-select: none; border: 10px none; box-sizing: border-box; display: inline-block; font-family: Roboto,sans-serif; cursor: pointer; text-decoration: none; margin: 0px 0px 0px 4px; padding: 0px; outline: medium none; font-size: inherit; font-weight: inherit; transform: translate(0px, 0px); height: 36px; line-height: 36px; min-width: 88px; color: rgb(90, 146, 191); transition: all 450ms cubic-bezier(0.23, 1, 0.32, 1) 0ms; border-radius: 2px; position: relative; overflow: hidden; background-color:#e0e0e0; text-align: center; width: 100px;" type="button" tabindex="0">
												...
												<div data-radium="true" style="margin-bottom:0px;" >
													<ul id="addGuestList" style="padding: 10px 4px 0px 4px;" data-radium="true"></ul>
												</div>
												<div data-radium="true">
											...
											<!-- <div><input type="checkbox" name="emailtoguest" id="emailtoguest" > Email RSVP detail to guests</div> -->
                                                                                    <button eid="<?php echo $topPropertyId; ?>" avail="<?php echo ($limitRsvp > 0)? ($limitRsvp - $attendees) :  0; ?>" id="guestCountButton" style="width: 120px; margin-top:10px; font-weight: bold; background-color: rgb(224, 224, 224); color: rgb(255, 255, 255); border-radius: 2px; border: medium none; font-size: 14px; height: 35px; cursor: not-allowed; outline: medium none;" data-radium="true" disabled="disabled" type="submit" class="gtm-medium">RSVP (<span id="guestCount">0</span>)</button>
										</form>
									</div>
								...
								<p class="gtm-thin gtm-thinn">Upcoming Events</p>
								...
								<?php
									...
									foreach($response as $key => $events){
                                                                            ...
                                                                            $upcomingtext = ($countevents > 1 ? 'UPCOMING EVENTS' : 'No More Events');
									    foreach($events as $event){
									...
									<div style="max-width:500px;  float: left; margin: 15px 0px; background: rgb(255, 255, 255) none repeat scroll 0% 0%; cursor: pointer; border-radius: 2px; overflow: hidden; box-shadow: 0px 1px 3px rgba(0, 0, 0, 0.12), 0px 1px 2px rgba(0, 0, 0, 0.24); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1) 0s; border: 1px solid rgb(187, 159, 125);" data-radium="true" class="rmq-fe6607cb">
											...
											<div style="width: 100%; padding: 20px;" data-radium="true">
												 <div class="upcoming-event-cls" data-radium="true">
                                                        <div style="text-transform: uppercase;" data-radium="true">
                                                            <p style="margin: 0px; margin-bottom: 10px; font-size: 40px; text-align: center" data-radium="true" class="gtm-bold">
                                                            <?php echo $day; ?></p>
												...
												<div style="display: inline-block; width: calc(100% - 100px); vertical-align: top; padding-left: 20px;" data-radium="true" class="text-left">
													<div style="margin-bottom: 10px;" data-radium="true">
														<div style="max-height: 4.2em; overflow: hidden; font-size: 20px; text-transform: uppercase;" data-radium="true" class="gtm-bold"><?php echo $event['title']; ?></div>
													</div>
													<div data-radium="true">
														<div data-radium="true" class="gtm-book">
															...
															<!-- /react-text --><!-- react-text: 201 --><?php echo (strtolower( $event['addressLine']) == 'knotel')? '': $event['addressLine']; ?><!-- /react-text -->
														</div>
													</div>
												</div>
											</div>
							...
							<div class="col-xs-12 col-md-5">
								<div style="padding:10px 52px 30px; width:100%; margin: 59px auto 40px 0px; color: rgb(255, 255, 255); background-color: rgb(187, 159, 125);" class="visible-md-block visible-lg-block">
									<p style="border-bottom: 2px solid #ffffff;font-size: 10em;margin: 0 0 10px 0;text-align: center;" class="gtm-bold">
									<?php echo $day1;  ?></p>
									<p style="clear: both;display: block;font-size: 85px;padding: 0; text-align: center;"><?php echo $monthabv1; ?></p>
									<!-- <p style="margin: 0px; font-size: 6em; font-weight: 300; text-align: center; text-transform: uppercase;" class="gtm-book"><?php echo $eventyear1; ?></p> -->
								</div>
								<div class="getintouch-cls">
									<p class="gtm-black" data-radium="true" style="font-size: 20px; margin-bottom:10px;">Join Our Events Mailing List</p>
									<?php echo do_shortcode("[contact-form-7 id='232' title='Event mailing list']");  ?>
									<p class="gtm-bold" data-radium="true" style="text-align: center; line-height: 24px; font-size: 14px;">
										CONTACT FOR MORE INFO
										<br>
										<span class="gtm-light" data-radium="true" style="font-size:14px;">
										(646)-979-0898
										<br>
			...
			padding: 0;
                ...
                <script>
                    $('form.wpcf7-form').submit(function(evt){
                        ...
                        $(this).append('<input type ="hidden" name = "formtype" value = "eventdetailmailinglist">');
                        $.ajax({
                            type: 'POST',
                            url: "<?php echo get_home_url(); ?>/wp-admin/admin-ajax.php",
